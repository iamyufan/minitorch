...
def default_log_fn(epoch, total_loss, correct, losses, time_per_epoch):
    print(
        f"Epoch: {epoch} \t loss: {total_loss} \t correct: {correct} \t time: {time_per_epoch}"
    )
    ...
    def train(self, data, learning_rate, max_epochs=500, log_fn=default_log_fn):
        ...
        for epoch in range(max_epochs):
            ...
            if epoch % 10 == 0 or epoch == max_epochs:
                time_per_epoch = total_time_epoch / 10
                total_time_epoch = 0.0
...
if __name__ == "__main__":
    ...
    backend_options = {
        "simple": SimpleTensorBackend,
        "cpu": FastTensorBackend,
        "gpu": GPUBackend,
    }

    FastTrain(HIDDEN, backend=backend_options[args.BACKEND]).train(data, RATE)
